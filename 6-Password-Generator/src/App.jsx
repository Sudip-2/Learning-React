  ...
  return (
    <>
      <div className='w-full h-svh bg-slate-900 flex justify-center items-start'>
        <div className='bg-slate-800 shadow-xl px-5 py-4 mt-5 rounded-lg w-[350px] sm:w-[500px]'>
          ...
          <div className='flex gap-1 justify-center sm:gap-3 mt-2'>
            <input type="range" min={6} max={20} value={length} onChange={(e) => setlenghth(e.target.value)} className='w-[90px]'/>
            <label htmlFor="length" className='text-white text-sm sm:text-base'>Length {length}</label>
            <input type="checkbox" defaultChecked={numberAllowed} onChange={() => setnumberAllowed((prev) => !prev)}/>
            <label htmlFor="length" className='text-white text-sm sm:text-base'>Numbers</label>
            <input type="checkbox" defaultChecked={charAllowed} onChange={() => setcharAllowed((prev) => !prev)}/>
            <label htmlFor="length" className='text-white text-sm sm:text-base'>Character</label>
          </div>
        </div>
      </div>
    </>
